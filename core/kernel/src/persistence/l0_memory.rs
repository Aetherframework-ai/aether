...
mod tests {
    ...
    async fn test_step_results() {
        ...
        store.save_step_result("wf1", "step1", b"result1".to_vec()).await.unwrap();
        store.save_step_result("wf1", "step2", b"result2".to_vec()).await.unwrap();
    ...
    async fn test_update_workflow_state() {
        ...
        store.update_workflow_state("wf1", WorkflowState::Running { current_step: None }).await.unwrap();
